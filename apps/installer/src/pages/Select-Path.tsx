...
export default function SelectPath() {
  ...
  return (
    <motion.div initial={{opacity: 0}} animate={{opacity: 1}} transition={{duration: 1}} className="flex justify-center items-center p-4 w-full h-full m-auto">
    <div className="w-full h-full flex flex-col max-w-2xl mt-auto justify-end pb-24 text-neutral-300 gap-2">
        <h1 className="text-xl font-semibold">Install in:</h1>
        <div className="w-full flex gap-4">
        <input value={value} placeholder="Loading..." readOnly className="shadow w-full cursor-default appearance-none rounded-lg px-4 py-1 bg-neutral-800/60 text-sm text-neutral-300 focus:outline-none"/>
        <button type="button" onClick={handleChange} className="w-fit text-sm px-4 py-1 rounded-xl bg-neutral-800/80 hover:bg-neutral-700/80 transition-all duration-200 border border-neutral-600/10 shadow">Change</button>
        </div>
