...
export default function InferencesLibrary() {
  ...
  const deleteInference = async (id: number) => {
    try {
      const port = await getServerPort();
      const response = await fetch(`http://localhost:${port}/delete-inference?id=${encodeURIComponent(id)}`);
      if (response.ok) {
        const data = await response.json();
  ...
  <div className="grid h-screen w-screen">
    <main className="flex flex-col items-center justify-start mt-10 mb-4 px-4 w-full overflow-auto">
      ...
      {loading ? (
        ...
        <>
        {inferences === 'No inferences found' ? (
          ...
          {inferences.map((inference: { id: number; model_name: string; converted_at: string; indexRate: number; filterRadius: number; autotune: boolean; cleanaudio: boolean; exportformat: string; audio_output: string }) => (
            ...
            >
              <div className="flex flex-row gap-2 w-full">
                ...
                <button
                  type="button"
                  className="rounded-lg border border-white/10 text-black p-3 flex items-center justify-center hover:bg-neutral-700/50 hover:shadow-xl hover:shadow-white/10 slow" onClick={() => downloadAudio(inference.audio_output)}>
                ...
                <button
                  type="button"
                  className="rounded-lg border border-white/10 text-black p-3 flex items-center justify-center hover:bg-red-700/20 hover:shadow-xl hover:shadow-red-700/10 slow" onClick={() => deleteInference(inference.id)}> 
