...
export default function AudioSelector() {
	...
	return (
		<div
			className={`max-h-80 mt-auto enabled:hover:opacity-100 relative shadow-xl ${audioBlob ? "" : "border border-white/10"} h-full w-full rounded-xl p-4 flex flex-col gap-2 justify-center items-center`}
		>
			<div className="absolute w-full h-full rounded-xl backdrop-blur-3xl backdrop-filter noise opacity-40" />
			{!audioUrl && (
				<div
					className="absolute top-4 left-4 border border-white/10 rounded-xl overflow-hidden"
					style={{ zIndex: 100 }}
				>
			...
			{uploaded || audioUrl ? (
				<>
					...
					<p className="absolute left-4 top-9 max-w-[200px] truncate text-xs text-neutral-400">
						{file?.name || input}
					</p>
				</>
			) : (
				<>
					{audioSection === "import" ? (
						...
						<>{!audioUrl && <Mic className="w-12 h-12" />}</>
					)}
				</>
			)}
			{!audioUrl && (
