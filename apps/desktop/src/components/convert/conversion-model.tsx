...
export default function ConversionModel() {
	...
	return (
		<div
			className="relative rounded-xl row-span-2 w-full h-14"
			ref={dropdownRef}
		>
			<div className="absolute w-full h-full rounded-xl border border-white/10" />
			{loading ? (
				...
				<>
					<div className="w-full h-full flex flex-col py-2 relative">
						...
						{isOpen && (
							<div
								className="absolute top-full left-0 w-full mt-2 bg-white/10 rounded-xl z-20 max-h-60 overflow-y-auto text-sm p-3 gap-2 flex flex-col shadow-xl"
								style={{ zIndex: 200 }}
							>
								{models.length > 0 ? (
									models.map((model) => (
										<div
											key={model.name}
											className="px-4 py-2 hover:bg-white/20 slow cursor-pointer text-neutral-300 rounded-xl border border-white/10 break-all"
			...
			{imagePath && (
				<div className="w-full h-fit border border-white/10 rounded-xl mt-4 p-4">
					...
					<p className="title text-2xl max-w-sm mt-6 truncate font-semibold text-neutral-200 text-center">
						{decodeURIComponent(currentModel.name)}
					</p>
				</div>
			)}
