...
class CommonSeparator:
        ...
        if (
            ...
            if instruments:
                ...
                if (
                    ...
                    and instruments[0] != target_instrument
                    and instruments[1] == target_instrument
                ):
                    self.logger.debug(
                        f"Swapping stem names: target_instrument '{target_instrument}' doesn't match instruments[0] '{instruments[0]}'"
                    )
        ...
        if not isinstance(mix, np.ndarray):
            ...
            try:
                ...
                if "PCM_16" in self.input_subtype or self.input_subtype == "PCM_S8":
                    ...
                    "PCM_32" in self.input_subtype
                    or "FLOAT" in self.input_subtype
                    or "DOUBLE" in self.input_subtype
                ):
                    ...
                    self.input_bit_depth = 16
                    self.logger.warning(
                        f"Unknown audio subtype {self.input_subtype}, defaulting to 16-bit output"
                    )

                self.logger.info(
                    f"Detected input bit depth: {self.input_bit_depth}-bit"
                )
    ...
    def write_audio_pydub(self, stem_path: str, stem_source):
        ...
        if self.output_dir:
            ...
            try:
                ...
                raise AudioExportError(
                    f"Failed to prepare output directory for {stem_path}: {e}",
                    path=stem_path,
                    backend="pydub",
                ) from e
        ...
        output_bit_depth = (
            self.input_bit_depth if self.input_bit_depth is not None else 16
        )
        ...
        try:
            ...
            raise AudioExportError(
                f"Failed to create audio for {stem_path} with pydub: {e}",
                path=stem_path,
                backend="pydub",
            ) from e
        ...
        with atomic_output_path(target_path, "pydub") as temp_path:
            ...
            if callable(close_export):
                close_export()
        self.logger.debug(
            f"Exported audio file successfully to {target_path} with {output_bit_depth}-bit depth"
        )
    ...
    def write_audio_soundfile(self, stem_path: str, stem_source):
        ...
        if self.input_subtype:
            ...
            self.logger.info(
                f"Using output subtype based on bit depth: {output_subtype}"
            )
        else:
        ...
        self.logger.debug(
            f"Exported audio file successfully to {target_path} with subtype {output_subtype}"
        )
    ...
    def get_stem_output_path(self, stem_name, custom_output_names):
        ...
        if custom_output_names:
            custom_output_names_lower = {
                k.lower(): v for k, v in custom_output_names.items()
            }
            stem_name_lower = stem_name.lower()
            if stem_name_lower in custom_output_names_lower:
                sanitized_custom_name = self.sanitize_filename(
                    custom_output_names_lower[stem_name_lower]
                )
                return os.path.join(
                    f"{sanitized_custom_name}.{self.output_format.lower()}"
                )
