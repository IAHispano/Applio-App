...
class ConfigurationNormalizer:
    ...
    def normalize_config(
        ...
        logger.debug(
            f"Normalized {model_type} configuration: {len(normalized)} parameters"
        )
        return normalized
    ...
    def detect_model_type(self, config: Dict[str, Any]) -> Optional[str]:
        ...
        if (
            ...
            and structural_type != explicit_type
        ):
            raise ParameterValidationError.incompatible_parameters(
    ...
    def detect_model_type_from_filename(self, file_path: str) -> Optional[str]:
        ...
        if "roformer" in filename:
            logger.warning(
                f"Generic 'roformer' detected in {filename}, defaulting to bs_roformer"
            )
            return "bs_roformer"
        return None
