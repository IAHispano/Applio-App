...
class BSRoformerValidator(ParameterValidator):
    ...
    def validate_stft_compatibility(
        ...
        if isinstance(freqs_per_bands, (list, tuple)) and all(
            isinstance(f, int) for f in freqs_per_bands
        ):
            total_freqs = sum(freqs_per_bands)
    ...
    def validate_mask_estimator_config(
        self, config: Dict[str, Any]
    ) -> List[ValidationIssue]:
        """
    ...
    def validate_all(
        self, config: Dict[str, Any], model_type: str = "bs_roformer"
    ) -> List[ValidationIssue]:
        """
    ...
    def get_parameter_defaults(self, model_type: str = "bs_roformer") -> Dict[str, Any]:
        ...
        defaults.update(
            {
                "freqs_per_bands": self.DEFAULT_FREQS_PER_BANDS[
                    :6
                ],  # Use first 6 bands as default
