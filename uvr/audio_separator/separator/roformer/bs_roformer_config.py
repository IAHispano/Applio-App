...
class BSRoformerConfig(ModelConfiguration):
    ...
    def _validate_bs_roformer_parameters(self):
        ...
        if not isinstance(self.freqs_per_bands, (tuple, list)):
            raise ValueError(
                f"freqs_per_bands must be a tuple or list, got {type(self.freqs_per_bands)}"
            )

        if not all(isinstance(freq, int) and freq > 0 for freq in self.freqs_per_bands):
            raise ValueError(
                "All frequencies in freqs_per_bands must be positive integers"
            )
        ...
        if self.mask_estimator_depth <= 0:
            raise ValueError(
                f"mask_estimator_depth must be positive, got {self.mask_estimator_depth}"
            )
