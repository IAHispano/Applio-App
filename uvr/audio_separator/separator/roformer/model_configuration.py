...
class ModelConfiguration:
    ...
    def _validate_basic_parameters(self):
        ...
        if not isinstance(self.num_stems, int) or self.num_stems <= 0:
            raise ValueError(
                f"num_stems must be a positive integer, got {self.num_stems}"
            )

        if (
            not isinstance(self.mlp_expansion_factor, int)
            or self.mlp_expansion_factor <= 0
        ):
            raise ValueError(
                f"mlp_expansion_factor must be a positive integer, got {self.mlp_expansion_factor}"
            )
    ...
    def _validate_parameter_ranges(self):
        ...
        if not (0.0 <= self.attn_dropout <= 1.0):
            raise ValueError(
                f"attn_dropout must be between 0.0 and 1.0, got {self.attn_dropout}"
            )

        if not (0.0 <= self.ff_dropout <= 1.0):
            raise ValueError(
                f"ff_dropout must be between 0.0 and 1.0, got {self.ff_dropout}"
            )
    ...
    def has_new_parameters(self) -> bool:
        ...
        return (
            ...
            or self.sage_attention is True
            or self.zero_dc is not True
            or self.use_torch_checkpoint is True
            or self.skip_connection is True
        )
