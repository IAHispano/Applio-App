...
class MelBandRoformerConfig(ModelConfiguration):
    ...
    fmax: Optional[float] = (
        None  # Maximum frequency for mel scale (None = sample_rate/2)
    )
    ...
    def _validate_mel_band_roformer_parameters(self):
        ...
        if not isinstance(self.num_bands, int) or self.num_bands <= 0:
            raise ValueError(
                f"num_bands must be a positive integer, got {self.num_bands}"
            )

        if self.num_bands < 8:
            raise ValueError(
                f"num_bands should be at least 8 for meaningful separation, got {self.num_bands}"
            )

        if self.num_bands > 512:
            raise ValueError(
                f"num_bands should not exceed 512 for practical purposes, got {self.num_bands}"
            )

        if self.mel_scale not in ["htk", "slaney"]:
            raise ValueError(
                f"mel_scale must be 'htk' or 'slaney', got '{self.mel_scale}'"
            )
        ...
        if self.fmax is not None:
            ...
            if self.fmax > self.sample_rate / 2:
                raise ValueError(
                    f"fmax ({self.fmax}) cannot exceed sample_rate/2 ({self.sample_rate/2})"
                )
