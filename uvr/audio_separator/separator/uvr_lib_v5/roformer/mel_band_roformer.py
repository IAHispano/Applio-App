...
class MelBandRoformer(Module):
    ...
    def forward(self, raw_audio, target=None, return_loss_breakdown=False):
        ...
        x_is_mps = (
            original_device.type == "mps"
            and should_fallback_to_cpu_for_complex_ops(original_device)
        )
        ...
        masks_summed = torch.zeros_like(
            stft_repr_expanded_stems.cpu() if x_is_mps else stft_repr_expanded_stems
        ).scatter_add_(
            2,
            scatter_indices.cpu() if x_is_mps else scatter_indices,
            masks.cpu() if x_is_mps else masks,
        )
        if not x_is_dml:
        ...
        recon_audio = torch.istft(
            ...
            **self.stft_kwargs,
            window=stft_window.cpu() if (x_is_mps or x_is_dml) else stft_window,
