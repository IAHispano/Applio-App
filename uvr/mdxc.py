...
class MdxcSeparator(BaseSeparator):
    def __init__(self, common_config, arch_config: dict):
        ...
        self.logger.debug(f"Model data keys: {list(self.model_data.keys()) if isinstance(self.model_data, dict) else None}")

        self.segment_size = arch_config.get("segment_size", 256)
        inference_config = self.model_data.get("inference", {}) if isinstance(self.model_data, dict) else {}
    ...
    def separate(self, audio_file_path, custom_output_names=None):
        ...
        for key, value in zip(instruments, source):
            if self.output_single_stem and self.output_single_stem.lower() != key.lower():
                continue
            stem_path = self.get_stem_output_path(key, custom_output_names)
            stem_source = spec_utils.normalize(wave=value, max_peak=self.normalization_threshold, min_peak=self.amplification_threshold).T
    ...
    def demix(self, mix):
        ...
        mix_t = torch.cat(
            [
                ...
                mix_t,
                torch.zeros(2, pad_size + chunk_size - hop_size, device=self.torch_device),
            ],
            1,
        )
        ...
        accumulated = torch.zeros(num_stems, *mix_t.shape, device=self.torch_device) if num_stems > 1 else torch.zeros_like(mix_t)
        with torch.no_grad():
            count = 0
            for batch in tqdm(batches, desc="Separating"):
                ...
                for individual in batch_result:
                    individual = individual.to(self.torch_device) if individual.device != self.torch_device else individual
                    accumulated[..., count * hop_size : count * hop_size + chunk_size] += individual
                    count += 1

        accumulated.div_(self.overlap)
        out = accumulated[..., chunk_size - hop_size : -(pad_size + chunk_size - hop_size)]
