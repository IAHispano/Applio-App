...
class BSRoformer(Module):
    ...
    def forward(
        self, raw_audio, target=None, active_stem_ids=None, return_loss_breakdown=False
    ):
        """
        einops
        ...
        for i, transformer_block in enumerate(self.layers):
            ...
            if len(transformer_block) == 3:
                ...
                x, ft_ps = pack([x], "b * d")
                if self.use_torch_checkpoint:
                    linear_out, _ = checkpoint(
                        linear_transformer, x, use_reentrant=False
                    )
                else:
            ...
            if self.use_torch_checkpoint:
                time_out, next_time_v_residual = checkpoint(
                    time_transformer, x, use_reentrant=False
                )
            else:
                time_out, next_time_v_residual = time_transformer(
            ...
            if self.use_torch_checkpoint:
                freq_out, next_freq_v_residual = checkpoint(
                    freq_transformer, x, use_reentrant=False
                )
            else:
                freq_out, next_freq_v_residual = freq_transformer(
        ...
        if self.use_torch_checkpoint:
            mask = torch.stack(
                [checkpoint(fn, x, use_reentrant=False) for fn in heads], dim=1
            )
        else:
        ...
        for window_size in self.multi_stft_resolutions_window_sizes:
            ...
            recon_Y = torch.stft(
                rearrange(recon_audio, "b n s t -> (b n s) t"), **res_stft_kwargs
            )
            target_Y = torch.stft(
                rearrange(target_sel, "b n s t -> (b n s) t"), **res_stft_kwargs
            )
