...
class ScaleTransformer(Module):
    ...
    def forward(self, x, band_embedding):
        ...
        for (norm1, attn, norm2, ff), ada in zip(self.layers, self.adaLN_modulations):
            ...
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                shift_scale.chunk(6, dim=-1)
            )
            x = x + attn(modulate(norm1(x), shift_msa, scale_msa)) * gate_msa.unsqueeze(
                1
            )
    ...
    def init_weights(self):
        ...
        GATE_BIAS_INIT = 1.0  # sigmoid(1.0)≈0.73
...
class BSRoformer_Conditional(Module):
