...
ARCH_CLASSES = {"vr": VrSeparator, "mdx": MdxSeparator, "demucs": DemucsSeparator, "roformer": RoformerSeparator, "mdxc": MdxcSeparator}
...
def _model_data_for(arch, weight_path):
    ...
    if arch in ("demucs", "roformer", "mdxc"):
        ...
        if yaml_path:
            ...
            target = training.get("target_instrument") or (instruments[0] if instruments else "Vocals")
            data.setdefault("primary_stem", target)
            return data
        return {"primary_stem": "Vocals"}
    digest = _md5(weight_path)
    table_file = os.path.join(here, "vr_model_data.json" if arch == "vr" else "mdx_model_data.json")
    with open(table_file, encoding="utf-8") as f:
        table = json.load(f)
    if digest not in table:
        raise ValueError(f"Unsupported {arch.upper()} weights: MD5 {digest} not in the vendored model data table.")
    return table[digest]


def _default_arch_params(arch):
    if arch == "vr":
        return {"batch_size": 1, "window_size": 512, "aggression": 5, "enable_tta": False, "enable_post_process": False, "post_process_threshold": 0.2, "high_end_process": False}
    if arch == "mdx":
        return {"hop_length": 1024, "segment_size": 256, "overlap": 0.25, "batch_size": 1, "enable_denoise": False}
    return {"segment_size": "Default", "shifts": 1, "overlap": 0.25, "segments_enabled": True}
...
def separate_stems(
    ...
    if model is None:
        raise ValueError(f"Unknown UVR model '{model_key}'. Available: {', '.join(sorted(catalog.BY_KEY))}")
    ...
    if model.arch == "demucs":
        ...
        if not yaml_path:
            raise ValueError(f"Demucs model '{model.key}' is missing its yaml config in {models_dir}.")
        weight_path = yaml_path
    elif model.arch in ("roformer", "mdxc"):
        yaml_path = _yaml_in_dir(models_dir)
        if not yaml_path:
            raise ValueError(f"Model '{model.key}' is missing its yaml config in {models_dir}.")
        roformer_config = yaml_path
    model_data = _model_data_for(model.arch, weight_path)
    if model.arch == "demucs":
        model_data = {**model_data, "primary_stem": model_data.get("primary_stem", "Vocals")}
    ...
    common_config = {
        ...
        "onnx_execution_provider": select_onnx_providers(use_gpu and torch_device.type == "cuda"),
        "model_name": model.key if model.arch in ("demucs", "roformer") else model.files[0][0].rsplit(".", 1)[0],
        ...
        "output_single_stem": None if str(single_stem).lower() == "all" else single_stem,
        "sample_rate": 44100,
    }
    arch_params = _default_arch_params(model.arch)
    if arch_overrides:
        arch_params = {**arch_params, **{k: v for k, v in arch_overrides.items() if v is not None}}
    ...
    for path in paths:
        ...
        full = path if os.path.isabs(path) else os.path.join(output_dir or repo_root, path)
